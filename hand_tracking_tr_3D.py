...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        for handLms in results.multi_hand_landmarks:
            ...
            # Finding the distance between tip of the thumb finger and tip of the ring finger
            thumb = handLms.landmark[4]
            ring = handLms.landmark[8]

            thumb_x = int(thumb.x * len(depth_image_flipped[0]))
            thumb_y = int(thumb.y * len(depth_image_flipped))
            ring_x = int(ring.x * len(depth_image_flipped[0]))
            ring_y = int(ring.y * len(depth_image_flipped))

            print(depth_image_flipped)

            images = cv2.circle(images, (ring_x, ring_y), 2, (255, 0, 0), -1)
            images = cv2.circle(images, (thumb_x, thumb_y), 2, (255, 0, 0), -1)

            thumb_distance = depth_image_flipped[thumb_y, thumb_x] * depth_scale
            ring_distance = depth_image_flipped[ring_y, ring_x] * depth_scale

            distance_thumb_ring = np.sqrt((thumb_distance - ring_distance) ** 2)

            images = cv2.putText(images, f"Dist_Thumb_Ring: {distance_thumb_ring}", (20, 200), font, fontScale, color,
